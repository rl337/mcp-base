...
class ExceptionMapper:
    ...
    def register(
        ...
        if message_formatter:
            ...
            def handler(exc: Exception) -> McpError:
                return McpError(ErrorData(code=error_code, message=message_formatter(exc)))
    ...
    def to_mcp_error(self, exception: Exception) -> McpError:
        ...
        if exc_type in self._mappings:
            return McpError(ErrorData(code=self._mappings[exc_type], message=str(exception)))

        # Default: internal error
        return McpError(
            ErrorData(code=McpErrorCode.INTERNAL_ERROR, message=f"Internal error: {str(exception)}")
        )
